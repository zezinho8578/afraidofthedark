...
function toggleAdminMode() {
    ...
    if (isAdmin) {
        ...
        } else if (pass) { alert('Invalid Credentials.'); }
    }
...
function getCompleteFormData() { 
    // Gather ID
    const data = { id: document.getElementById('character-id').value || `file_${Date.now()}` }; 
    data.branchRankLocked = document.getElementById('lock-branch-rank-checkbox').checked; 
    ...
    // Gather Lists
    data.crafts = Array.from(document.querySelectorAll('#craft-skills-container .sub-skill-item')).map(item => ({checked: item.querySelector('input[type=checkbox]').checked, type: item.querySelector('select').value, score: item.querySelector('input[type=number]').value})); 
    data.languages = Array.from(document.querySelectorAll('#language-skills-container .sub-skill-item')).map(item => ({checked: item.querySelector('input[type=checkbox]').checked, lang: item.querySelector('input[type=text]').value, score: item.querySelector('input[type=number]').value})); 
    data.weapons = Array.from(document.querySelectorAll('#weapons-body tr')).map(row => { const i = row.querySelectorAll('input'); return { name: i[0].value, skill: i[1].value, range: i[2].value, dmg: i[3].value, ap: i[4].value, lethality: i[5].value, ammoC: i[6].value, ammoM: i[7].value }; }); 
    data.bonds = Array.from(document.querySelectorAll('.bond-item')).map(item => ({ checked: item.querySelector('input[type="checkbox"]').checked, name: item.querySelector('input[type="text"]').value, score: item.querySelector('select').value })); 
...
function populateCompleteForm(data) {
    ...
    // Fill standard inputs (ignoring missing fields like 'team')
    Object.keys(data).forEach(key => { 
        if (!['weapons', 'bonds', 'crafts', 'languages', 'branchRankLocked', 'isArmorActive', 'currentArmorPoints'].includes(key)) { 
            const el = document.getElementById(key); 
            if (el) { el[el.type === 'checkbox' ? 'checked' : 'value'] = data[key]; } 
        } 
    });
    
    document.getElementById('bonds-container').innerHTML = ''; 
    if (data.crafts) data.crafts.forEach(c => addCraftSkill(c)); 
    if (data.languages) data.languages.forEach(l => addLanguageSkill(l));
    ...
    updateAllCalculations(); checkAdaptedStatus(); updateBranchRankLockState();
...
function saveCharacter() { const data = getCompleteFormData(); const name = data['full-name'] || 'UNKNOWN_SUBJECT'; const sheetName = prompt("Enter Casefile Name:", name); if (sheetName) { localStorage.setItem(data.id, JSON.stringify({ name: sheetName, ...data })); alert(`File '${sheetName}' archived locally.`); loadSavedSheets(); } }
function loadSavedSheets() { const list = document.getElementById('saved-sheets-list'); list.innerHTML = ''; for (let i = 0; i < localStorage.length; i++) { const key = localStorage.key(i); if (key.startsWith('file_') || key.startsWith('char_')) { const data = JSON.parse(localStorage.getItem(key)); const li = document.createElement('li'); li.innerHTML = `<span>${data.name}</span><div><button onclick="renameSheet('${key}')">RENAME</button><button onclick="loadSheet('${key}')">ACCESS</button><button class="remove-btn" onclick="deleteSheet('${key}')">PURGE</button></div>`; list.appendChild(li); } } }
function loadSheet(key) { populateCompleteForm(JSON.parse(localStorage.getItem(key))); showView('character-sheet'); }
function deleteSheet(key) { if(confirm("PERMANENTLY DELETE RECORD?")){ localStorage.removeItem(key); loadSavedSheets(); }}
...
function screenshotPage(event) {
    ...
    html2canvas(document.querySelector('.container'), { useCORS: true, allowTaint: true, scrollX: 0, scrollY: -window.scrollY })
        .then(canvas => {
            ...
            const agentName = document.getElementById('full-name').value || 'UNKNOWN_SUBJECT';
            a.download = `AOTD_${agentName.replace(/\s+/g, '_')}_Record.png`;
            a.click(); a.remove();
        }).catch(err => { console.error('Screenshot failed:', err); alert('Capture error. Check console.');
